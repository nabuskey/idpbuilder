...
func (r *LocalbuildReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err = ensureNamespace(ctx, r.Client, nginxNamespace); err != nil {
		return ctrl.Result{}, err
	}

	if r.Config.SelfSignedCert == "" {
		cert, _, err := util.GetOrCreateIngressCertificateAndKey(ctx, r.Client, util.SelfSignedCertSecretName, nginxNamespace)
		if err != nil {
			return ctrl.Result{}, err
		}
